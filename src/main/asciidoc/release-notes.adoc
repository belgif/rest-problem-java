...
* Use <<io.github.belgif.rest.problem.json-pointer-enabled, JSON Pointer syntax>> by default for input validation issues in request body.
...
[WARNING]
====
*Breaking change:* Classes were moved to `boot`-specific package
====
...
[WARNING]
====
*Breaking change:* Classes were moved to `client`-, `core`- and `server`-specific packages
====
...
* Users of `belgif-rest-problem-spring-boot-4` should now depend on the `belgif-rest-problem-spring-boot-4-starter` module.
...
[WARNING]
====
*Breaking change:* The `<classifier>jakarta</classifier>` variant of `belgif-rest-problem-java-ee` no longer exists and should be replaced by `belgif-rest-problem-jakarta-ee`.
====
...
*belgif-rest-problem-spring-boot-4:* Added new module for Spring Boot 4 and Jackson 3

*belgif-rest-problem-spring:* renamed to belgif-rest-problem-spring-boot-common
