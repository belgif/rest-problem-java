...
* *belgif-rest-problem-quarkus*: Quarkus integration for belgif-rest-problem (only in JVM mode)
...
[WARNING]
====
Currently, Quarkus integration is only supported in *JVM mode*, and not in native mode.
====
...
On Jakarta EE, a default implementation is provided that retrieved the locale from a thread-local variable that gets initialized with the value from the Accept-Language header.
