...
* *belgif-rest-problem-spring*: Spring integration for belgif-rest-problem
* *belgif-rest-problem-spring-boot-3-starter*: Spring Boot 3.x integration for belgif-rest-problem
* *belgif-rest-problem-spring-boot-4-starter*: Spring Boot 4.x integration for belgif-rest-problem
...
----
compile 'io.github.belgif.rest.problem:belgif-rest-problem-spring-boot-3-starter:{revnumber}'
----
...
----
compile 'io.github.belgif.rest.problem:belgif-rest-problem-spring-boot-4-starter:{revnumber}'
----
...
<dependency>
  <groupId>io.github.belgif.rest.problem</groupId>
  <artifactId>belgif-rest-problem-jakarta-ee</artifactId>
  <version>{revnumber}</version>
</dependency>
----
...
If your application uses Spring without Spring Boot, use the link:apidocs/io/github/belgif/rest/problem/spring/EnableProblemModule.html[`@EnableProblemModule`] annotation to activate problem support.
Your application needs to provide a configured `ObjectMapper` (Jackson 2) or `JsonMapper` (Jackson3) bean.
REST clients have to be configured during their creation using beans provided by the problem module.
See the https://github.com/belgif/rest-problem-java/blob/main/belgif-rest-problem-it/belgif-rest-problem-spring-it/src/main/java/io/github/belgif/rest/problem/it/WebConfig.java[`WebConfig`] class in the integration test `belgif-rest-problem-spring-it` for an example.
...
To implement an input-aware JAX-RS ParamConverter, extend from `io.github.belgif.rest.problem.ee.server.jaxrs.AbstractInputParamConverterProvider`.
...
Enable using the standard https://datatracker.ietf.org/doc/html/rfc6901[JSON Pointer] syntax for the "name" property of input validation issues in the request body.
Default `true`.
  ...
  "issues": [
    {
      ...
      "detail": "must match \"^\\d{11}$\"",
      "in": "body",
      "name": "/persons/0/ssin", <1>
      "value": "1234"
    }
  ]
}
...
<1> Standard JSON Pointer syntax

When set to `false`, the legacy format is used.
This is a variant of the https://datatracker.ietf.org/doc/html/rfc9535[JSONPath] syntax compact dot notation, where the leading `$.` is omitted.
...
<1> Legacy JSONPath syntax

This flag applies to all input validation issues generated by the library (e.g. schema validation).
For input validation issues with user-defined inputs, this flag also enables a best-effort automatic conversion to the JSON Pointer syntax.
A warning is logged when such automatic conversion is performed, because it is recommended that users update their code to use the new JSON Pointer syntax.

```java
new RequestValidator().ssin(
        Input.body("ssin", "99999999999")).validate();
```

```
WARNING io.github.belgif.rest.problem.json-pointer-enabled is enabled, auto-converting 'ssin' to JSON Pointer format '/ssin'
```
